...
const menu = [
  ...
  { text: "数据源", href: "/datasets", icon: "material-symbols-light:dataset-linked-outline-sharp" },
...
<template>
  <main class="flex h-screen">
    ...
    <div class="w-[calc(100%-70px)] bg-white dark:bg-gray-900/90">
      <!-- windows 定制化窗口按钮 -->
      <div v-if="isNotMac" class="flex h-10 bg-gray-400" style="--wails-draggable: drag">
        <div class="grow"></div>
        <div class="flex">
        <button
          ...
          @click="WindowMinimise" style="--wails-draggable: none">
          <Icon icon="mdi:window-minimize" />
        </button>
        <button
          class="w-10 h-10 text-xl flex flex-col items-center justify-center hover:bg-[#E9E9E9] dark:hover:bg-[#2D2D2D]"
          @click="isMaximised ? (WindowUnmaximise(), isMaximised = false) : (WindowMaximise(), isMaximised = true)"
        ...
        <button
          ...
          @click="Quit" style="--wails-draggable: none">
    ...
    <!-- 页面内容 -->
    <router-view class="overflow-y-auto h-screen" style="--wails-draggable:none;" />
    </div>
  </main>
</template>
